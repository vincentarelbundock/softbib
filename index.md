...
function to detect `R` packages used in a project folder. Like the
`renv` package, `softbib` can respect user instructions to ignore
certain files and folders. To specify those ignore instructions, you
must place a file called `.renvignore` in the working directory. This
file can include lines such as:
...
For a `BibLaTeX` version of this example, see [this
issue.](https://github.com/vincentarelbundock/softbib/issues/12)
